...
def export(brand):
    ...
    worksheet.write('I1', 'Product Code')
    worksheet.write('J1', 'Specifications')
    worksheet.write('K1', 'Cross Reference')
    worksheet.write('L1', 'Applications')
    worksheet.write('M1', 'Image URL')
    worksheet.write('N1', 'Product URL')

    worksheet.set_column('A:I', 15)
    worksheet.set_column('J:N', 30)
                ...
                for item in data_file:
                    ...
                    for category_item in category_list:
                        for product_item in item[category_item]:
                            ...
                            worksheet.write(row, col + 8,  product_item['code'])
                            worksheet.write(row, col + 9, flatten_list(details['specifications']))
                            worksheet.write(row, col + 10, flatten_list(details['cross_reference']))
                            worksheet.write(row, col + 11, flatten_list(details['applications']))
                            worksheet.write(row, col + 12,  details['image_url'])
                            worksheet.write(row, col + 13,  details['url'])
